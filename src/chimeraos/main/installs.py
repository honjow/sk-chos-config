...
from utils import (
    get_github_clone_cdn,
    get_github_raw_cdn,
    get_github_release_cdn,
    run_command,
    toggle_service,
)
...
def handycon_install_old():
    ...
    os.system(
        "sudo pacman -Sy python-installer python-build python-wheel python-setuptools --needed --noconfirm"
    )
    ...
    if os.path.exists(git_directory):
        ...
        command = (
            "mkdir -p ~/.cache/sk-holoiso-config/git "
...
def handycon_uninstall_old():
    ...
    command = (
        "sudo systemctl stop handycon && sudo systemctl disable handycon;"
...
def hhd_uninstall():
    ...
    hhd_owner_comand = "LANG=en_US pacman -Qo {} 2>/dev/null | awk '{print $5}'".format(
        hhd_path
    )
...
def decky_update_callback():
    ...
    script_path = os.path.expanduser(
        "~/.cache/sk-holoiso-config/user_install_script.sh"
    )
...
def simple_decky_install():
    ...
    # if github_cdn_url:
    #     git_url = git_url.replace("https://github.com", github_cdn_url)
    command = 'curl -sLk {} | sed \'s#prerelease == "false"#prerelease == "true"#\' | sh'.format(
        git_url
    )
...
def decky_plugin_update(git_url, p_name=None):
    ...
    if os.path.exists(repo_directory):
        ...
        upt_command = ("mkdir -p {} && cd {} && git clone {}").format(
            git_directory, git_directory, git_url
        )
    ...
    build_command = "cd {} && rm -rf node_modules pnpm-lock.yaml && pnpm i && pnpm update decky-frontend-lib --latest && pnpm run build".format(
        repo_directory
    )
    ...
    plugin_directory = os.path.expanduser(
        "{}/{}/".format(plugin_parent_directory, plugin_name)
    )
    ...
    deploy_command = (
        f"chmod -v 755 {plugin_parent_directory} "
...
def remove_decky_plugin(plugin_name):
    command = "sudo rm -rf ~/homebrew/plugins/{} && sudo systemctl restart plugin_loader.service ".format(
        plugin_name
    )
...
def emudeck_install():
    ...
    command = (
        f"bash {SK_TOOL_SCRIPTS_PATH}/emudeck_install.sh {release_prefix} {raw_prefix}"
    )
...
def an_anime_game_launcher_install():
    release_prefix = get_github_release_cdn()
    raw_prefix = get_github_raw_cdn()
