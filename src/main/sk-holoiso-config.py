...
class SkHoloisoConfigApp(Gtk.Application):
    ...
    def on_activate(self, app):
        ...
        switch_item_handycon = SwitchItem("HandyGCCS", "用来驱动部分掌机的手柄按钮", handycon_enabled, install.handycon_switch_callback)
        ...
        if self.product_name == "ONEXPLAYER 2 ARP23":
            ...
            switch_item_oxp2lsusb = SwitchItem("OXP2手柄热插拔检测修复", "修复OXP2手柄热插拔后不识别的问题", oxp2lsusb_enabled, install.oxp2lsusb_switch_callback)
            ...
            switch_item_oxp2_volume_button_fix = SwitchItem("OXP2音量键修复", "修复OXP2音量键问题", oxp2_volume_button_fix_enabled, install.oxp2_volume_button_fix_switch_callback)
        ...
        if self.product_name in (
            ...
            switch_item_aya_lc_suspend = SwitchItem("AYANEO LC键睡眠", "默认为截图, 开启后LC键作为睡眠键", aya_lc_suspend_enabled, install.aya_lc_suspend_switch_callback)
